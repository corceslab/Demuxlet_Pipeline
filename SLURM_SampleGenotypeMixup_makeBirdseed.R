...
colnames(affx) <- c("hg38_chr","hg38_start","hg38_stop","Allele_A","Allele_B")
affx$Probe_Set_ID <- rownames(affx)

#make rownames indexable by joining chr and stop position
rownames(affx) <- paste(affx$hg38_chr,affx$hg38_stop,sep="-")
...
affxSNPs$chr <- affx$hg38_chr
affxSNPs$pos <- affx$hg38_stop
